...
1)	Документація testrail (для ознайомлення) https://support.testrail.com/hc/en-us/articles/7077083596436-Introduction-to-the-TestRail-API
...
5)	Ознайомитися з можливими статусами результатів виконання тесткейсу GET http://testrail01.slygods.top/index.php?/api/v2/get_statuses
"""


"""ЗМІННІ ЯКІ ПОТРІБНО ЗАПОВНИТИ!!!"""

token_testrail = config('Basic Z29tZWhlMzg1M0BpbnRyb2FjZS5jb206Rnc1L0JHa3dEV2dGR2pnQ2UvcXotQjlkR08yT25KUGl2d05LbnVTQWQ=') # вказати токен з яким ви заходите в тестреіл.
run_id = 6591 # потрібно вказати значення Test Run з тестреіл. наприклад  run_id = 6498 тут http://testrail01.slygods.top/index.php?/runs/view/6498
...
@pytest.fixture
def request_testrail(current_time_utc): # використовуємо фікстуру current_time_utc для вказання в коментарях часу виконання тесту
...
def test_login_page(logger_fixture, request_testrail): # використовую 2 фікстури
    ...
    case_id = 197727  # ID test case в testrail
    try:
        result = LoginPage().driver_login_page() # записую результат виконання теста
        result_in_comment = request_testrail(result, expected_result, case_id) # отримую статус виконання тесту (для відобораження результату в логах) та передаю в фікстуру необхідні параметри
        ...
        result_in_comment = request_testrail(result, expected_result, case_id) # отримую статус виконання тесту (для відобораження результату в логах) та передаю в фікстуру необхідні параметри
        log.info(f'Тест має статус виконання "{result_in_comment}", помилка {str(e)}, був запущений') # в логи записую статус виконання та саму помилку
        pytest.fail(f'Тест не вдалося виконати через помилку: {str(e)}') # позначаю що даний тест неуспішний та вивожу помилку
